              ...
              <x-slot:tablehead>
                <tr class="border bg-neutral-100">
                  ...
                  <x-table-th class="text-start">
                    marca/volumen
                    <x-quest-icon
                      title="kilogramos (K), gramos (g), litros (L), mililitros (mL), metros (M), centimetros (cm), unidades (U)" />
                  </x-table-th>
                  <x-table-th class="text-end">
                    presupuestar
                  ...
                  <x-table-th class="text-end">
                    $precio unitario
                    <x-quest-icon
                      title="precio unitario de un suministro o pack, ejemplo: 1230.20 (mil doscientos treinta con veinte)" />
                    ...
                    @if ($input['item_type'] === 'suministro')
                      ...
                      <x-table-td class="text-end">
                        <div class="flex flex-col w-full gap-1">
                          <div class="flex items-center justify-start w-full gap-1">
                            <span>$&nbsp;</span>
                            <input type="text" id="input_{{ $key }}_item_unit_price"
                              wire:model.live="inputs.{{ $key }}.item_unit_price"
                              wire:change="calculateSubtotal({{ $key }}, $event.target.value)" pattern="[0-9.,]*"
                              class="w-32 p-1 text-sm text-right border grow border-neutral-200 @error('inputs.' . $key . '.item_unit_price') border-red-200 @enderror focus:outline-none focus:ring focus:ring-neutral-300"
                              placeholder="precio unitario" autocomplete="off" />
                          </div>
                        </div>
                      </x-table-td>
                      {{-- precio subtotal --}}
                      <x-table-td class="text-end">
                        <div class="flex flex-col w-full gap-1">
                          <div class="flex items-center justify-start w-full gap-1">
                            <span>$&nbsp;</span>
                            <input type="text" id="input_{{ $key }}_item_total_price"
                              wire:model.live="inputs.{{ $key }}.item_total_price" pattern="[0-9.,]*"
                              wire:change="formatSubtotal({{ $key }}, $event.target.value)"
                              class="w-32 p-1 text-sm text-right border grow border-neutral-200 @error('inputs.' . $key . '.item_total_price') border-red-200 @enderror focus:outline-none focus:ring focus:ring-neutral-300"
                              placeholder="precio subtotal" autocomplete="off" />
                          </div>
                        </div>
                      </x-table-td>
                    @else
